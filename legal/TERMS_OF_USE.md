# Terms of Use — Relo Dojo

**Effective date:** [INSERT DATE]
**Operator ("we", "us"):** [INSERT LEGAL NAME / SOLE PROPRIETOR] ("Relo Dojo")
**Contact:** [INSERT CONTACT EMAIL]

> Draft prepared for the app stores. Complete the bracketed fields and the governing-law section, and
> have a qualified lawyer review before publication. This is not legal advice.

By creating an account or using Relo Dojo (the "App"), you agree to these Terms. If you do not
agree, do not use the App.

## 1. The service

Relo Dojo provides English-learning exercises, automated feedback, progress tracking, and
gamification features (experience points, streaks, a virtual in-app currency called "koku", and
cosmetic items). The App is provided for personal, non-commercial educational use.

## 2. Eligibility & accounts

You must be at least [13 / 16 — choose per your markets] years old, or the minimum age of digital
consent in your country, to use the App. You are responsible for the accuracy of your registration
details and for keeping your password confidential. You are responsible for activity under your
account. Notify us promptly of any unauthorized use.

## 3. Acceptable use

You agree not to: (a) reverse engineer, tamper with, or attempt to bypass the App's metering,
gating, or economy; (b) submit content that is unlawful, abusive, infringing, or that attempts to
manipulate the automated tutor ("prompt injection"); (c) use automated means to access the service
beyond normal personal use, or attempt to exceed rate limits; (d) attempt to gain unauthorized access
to other accounts or our systems.

## 4. Automated feedback — no guarantee of accuracy

Exercises, grading, and explanations are generated in part by automated language models. They may
occasionally be incorrect, incomplete, or unsuitable. The App is a learning aid, not a substitute for
professional instruction or certification. Use your judgment and verify important information.

## 5. Virtual items and currency ("koku")

Koku, charms, cosmetics, and similar items are a limited, personal, non-transferable license to use
features within the App. They have **no monetary value**, cannot be exchanged for real money, and
are not redeemable outside the App. We may adjust, expire, or remove virtual items, balances, prices,
and earning rules to balance the game or for operational reasons. Time-limited offers expire as
stated and are not recoverable once expired.

## 6. Purchases

Relo Dojo offers optional premium access ("Black Belt"). Premium is a **one-time, non-renewing** paid
access that you buy from within the App: the App opens a secure web checkout and the payment is
processed by our payment provider **YooKassa (ЮMoney / YooKassa)** under its terms. It is not a
subscription and does not auto-renew; access lasts for the period stated for the plan you buy. Prices
are shown before you pay. "Premium" provides only the benefits described in the App at the time of
purchase; we may change what future premium plans include.

Refunds are handled by us in accordance with applicable law, including, for consumers in Russia, the
Law "On Protection of Consumer Rights" (Закон «О защите прав потребителей», ЗоЗПП). To request a
refund or raise a payment issue, contact us at [INSERT CONTACT EMAIL].

## 7. Your content

You retain ownership of the text you submit (answers, goals, text you choose to review). You grant us
a limited license to process that content solely to operate and improve the service as described in
the Privacy Policy. Do not submit content you are not entitled to share or that contains sensitive
personal data you do not want processed.

## 8. Intellectual property

The App, its design, mascot, content, and software are owned by the operator or its licensors and are
protected by law. We grant you a personal, revocable, non-exclusive, non-transferable license to use
the App. You may not copy, distribute, or create derivative works except as permitted by law.

## 9. Availability & changes

We may modify, suspend, or discontinue any part of the App at any time. We may update these Terms;
material changes will be notified in the App or by updating the effective date. Continued use after
changes means you accept them.

## 10. Termination

You may stop using the App and delete your account at any time. We may suspend or terminate access if
you violate these Terms or to protect the service. On termination, your license to use the App ends.

## 11. Disclaimers

The App is provided "as is" and "as available" without warranties of any kind, to the maximum extent
permitted by law. We do not warrant that the App will be uninterrupted, error-free, or that automated
feedback will be accurate.

## 12. Limitation of liability

To the maximum extent permitted by law, the operator will not be liable for indirect, incidental,
special, or consequential damages, or for loss of data, progress, or virtual items. [Operator: add any
jurisdiction-specific liability cap, e.g. limited to amounts you paid in the prior 12 months, where
permitted.] Nothing limits liability that cannot be limited by law.

## 13. Governing law & disputes

These Terms are governed by the laws of [INSERT JURISDICTION], without regard to conflict-of-laws
rules. Disputes will be resolved in the courts of [INSERT VENUE], unless mandatory local law gives you
other rights.

## 14. Contact

[INSERT LEGAL NAME] — [INSERT CONTACT EMAIL]
